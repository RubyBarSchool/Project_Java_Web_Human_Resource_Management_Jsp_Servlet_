<%-- 
    Document   : UpdateChamCong
    Created on : Apr 2, 2020, 4:23:45 PM
    Author     : lenovo
--%>
        ...
        <div class="container-fluid" >
            ...
            <div class=" full row-cols-5">
                ...
                <div class="float-right col-10"  >
                    <form action="update_chamcong" method="POST" >
                        <table border="1px" class="table-responsive" >
                            ...
                            <tr>
                                <c:set value="${requestScope.nhanvien}" var="nv1" ></c:set>
                                <td>${nv1.id}</td>
                                <td>${nv1.name}</td>
                                <c:forEach items="${requestScope.arrdate}" var="date" > 
                                    <c:set value="0" var="check" ></c:set>
                                    <c:forEach items="${requestScope.arrChamCong}" var="cc" >
                                        <c:if test="${date eq cc}" >                                                        
                                            <c:set value="1" var="check" ></c:set>                                          
                                        </c:if>
                                    </c:forEach>
                                    <td class="text-center" ><input type="checkbox" ${(check eq 1)?"checked":""} value="${date}"  name="chamcong" /></td>
                                    </c:forEach>     
                            </tr>
                        </table>
